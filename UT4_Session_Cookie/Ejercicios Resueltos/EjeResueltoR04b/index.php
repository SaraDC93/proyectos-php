...
if (!isset($_SERVER['PHP_AUTH_USER'])) {  //para mandar el login
    //si no tiene valor
    ...
    $consulta->execute();
    $resultado = $consulta->get_result(); //como un fetch

    // Si no existe el usuario o la contraseña es incorrecta, pedimos las credenciales de nuevo
    if ($resultado->num_rows == 0) { //si en la variable resultado el numero de filas es 0
        header('WWW-Authenticate: Basic realm="Contenido restringido"');
        header("HTTP/1.0 401 Unauthorized");
        exit;
    } else {
